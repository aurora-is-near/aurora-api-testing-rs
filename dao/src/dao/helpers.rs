...
impl TransactionReceipt {
    pub fn load(raw_transactions_data: Vec<String>) -> Result<Vec<TransactionReceipt>, SerdeError> {
        let receipts = raw_transactions_data
            .into_iter()
            .map(|r: String| -> TransactionReceipt { serde_json::from_str(&r.clone()).unwrap() })
            .collect();
        Ok(receipts)
    }
}
